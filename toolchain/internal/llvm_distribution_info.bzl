...
# Keys from above which are used to reduce hard-coded release name logic.
def _release_names(distributions):
    relnames = {}

    prefix = "clang+llvm-"
    suffix = (".tar.xz", ".tar.gz")

    split = lambda dist: (
        dist.removeprefix(prefix).removesuffix(suffix[0]).removesuffix(suffix[1]).split("-", 2)
    )

    for dist in distributions:
        if not (dist.startswith(prefix) and dist.endswith(suffix)):
            fail("Invalid assumption for distribution key. Please fix me!: " + dist)
        version, arch, name = split(dist)
        relnames.setdefault(version + "-"  + arch, []).append(name)

    return relnames

# Contains the list of release names for keys of the form "{version}-{arch}"
LLVM_PATCH_RELEASE_NAMES = _release_names(LLVM_DISTRIBUTIONS.keys())
