...
def load_distribution_files(rctx, srcs):
    """Load and merge distribution JSONC files in declaration order.

    Returns `(distributions, distribution_urls)`. Later files replace earlier
    checksums. A later URL replaces an earlier URL only when the later file
    supplies a covering `_meta.base_url` template.
    """
    distributions = {}
    distribution_urls = {}
    for src in srcs:
    ...
    return distributions, distribution_urls

def _impl(rctx):
    distributions, distribution_urls = load_distribution_files(rctx, rctx.attr.srcs)

    header = "\n".join([
        "# Auto-generated by //toolchain/internal:distributions_repo.bzl.",
        "# Do not edit by hand. Sources:",
    ] + ["#   - " + str(src) for src in rctx.attr.srcs]) + "\n\n"
